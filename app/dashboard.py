...
with tab1:
    ...
    with col4:
        safety_score = round(np.mean([braking_data['intensity'].mean(), swerving_data['intensity'].mean()]), 1) if not (braking_data.empty or swerving_data.empty) else 0
    ...
    if time_series_data.empty:
        ...
        try:
            ...
            st.error(f"Error creating incident trends chart: {str(e)}")
...
with tab4:
    ...
    try:
        ...
        st.error(f"Error generating anomaly detection: {str(e)}")
